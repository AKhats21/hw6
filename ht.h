...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{
    for (size_t i = 0; i < table_.size(); i++) {
        if (table_[i] == nullptr || table_[i]->deleted == true) continue;
        delete table_[i];
    }
}
...
{
    HashItem *insert = new HashItem(p);


    if ((double)size()/table_.size() >= resizeAlpha_) resize();
    // std::cout << p.first << std::endl;
    if (probe(p.first) == npos) throw std::logic_error("Key Not Found");
    if (table_[probe(p.first)] == nullptr) table_[probe(p.first)] = insert;
    else if (table_[probe(p.first)]->deleted == true) {
        HASH_INDEX_T h = hash_(p.first) % CAPACITIES[mIndex_];
        prober_.init(h, CAPACITIES[mIndex_], p.first);

        HASH_INDEX_T loc = prober_.next(); 
        totalProbes_++;
        while(table_[loc]->deleted != true)
        {
...
{
    // 1. Check if we can resize further
    if (mIndex_ + 1 >= sizeof(CAPACITIES)/sizeof(HASH_INDEX_T)) {
        throw std::logic_error("No more capacities available");
    }

    // 2. Store the old table and its size
    std::vector<HashItem*> oldTable = table_;
    size_t oldSize = table_.size();

    // 3. Create new empty table with the next capacity
    mIndex_++;
    table_.clear();  // Clear current table
    table_.resize(CAPACITIES[mIndex_], nullptr);  // Resize and initialize to nullptr

    // 4. Rehash all non-deleted items
    for (size_t i = 0; i < oldSize; ++i) {
        if (oldTable[i] != nullptr && !oldTable[i]->deleted) {
            ...
            delete oldTable[i];  // Delete marked items
        }
    }
